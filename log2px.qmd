...
Title: pxmake - multilingual px files with R, 2024 ([download this script](log2px.qmd))
...
... or just before further analysis, the dataframe 'statlog' can be transformed to a px-file for each selected variable. Footnotes in the px-files can be used to present additional information on the data. 
The px-files will be stored in a folder, along with needed Alias-text files for presentation in Pxweb.
...
biannual as:  
time = ifelse(month(as.Date(timestamp)) <= 6, paste0(year(as.Date(timestamp)), "H1"),
